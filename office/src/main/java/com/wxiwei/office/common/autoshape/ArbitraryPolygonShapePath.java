...
public class ArbitraryPolygonShapePath
{
        ...
        Element sp, BackgroundAndFill fill, boolean border, BackgroundAndFill lineFill, Element ln, Rectangle rect) throws Exception
    {
        ...
        if (ln != null)
        {
            ...
            {
                lineWidth = Math.round(IntegerTool.parseIntWithoutException(ln.attributeValue("w")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
            }
        }
        ...
        if (pathElements.size() == 1 && ln != null)
        {
             ...
             {
                 float w = IntegerTool.parseIntWithoutException(width) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                 float h = IntegerTool.parseIntWithoutException(height) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                        ...
                        if(path1 != null)
                        {
                            ...
                            if (fill != null || border)
                            {
                                if (border && 
                                    ...
                                    || IntegerTool.parseIntWithoutException(pathElement1.attributeValue("stroke")) != 0))
                                {
            ...
            if (fill != null || border)
            {
                ...
                if (border)
                {
                    if (pathElement.attribute("stroke") != null 
                        && IntegerTool.parseIntWithoutException(pathElement.attributeValue("stroke")) == 0)
                    {
        ...
        for(int i = 0; i < cnt; i++)
        {
        	...
        	else if(tailArrowTailCenter != null && i == cnt - 1)
        	{
                ...
                else if(e.getName().equals("quadBezTo"))
                {
                    ...
                    path.quadTo(IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("y")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                ...
                else if(e.getName().equals("cubicBezTo"))
                {
                    ...
                    path.cubicTo(IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("y")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("x") )* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("y")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        tailArrowTailCenter.x,
                        tailArrowTailCenter.y);
                }
                else if(e.getName().equals("arcTo"))
                {
                    float wR = IntegerTool.parseIntWithoutException(e.attributeValue("wR")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                    float hR = IntegerTool.parseIntWithoutException(e.attributeValue("hR"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                    ...
                    path.arcTo(new RectF((float)rect.getCenterX() - wR - rect.x, (float)rect.getCenterY() - hR - rect.y, (float)rect.getCenterX() + wR - rect.x, (float)rect.getCenterY() + hR - rect.y), 
                        IntegerTool.parseIntWithoutException(e.attributeValue("stAng")) / 60000f,
                        IntegerTool.parseIntWithoutException(e.attributeValue("swAng")) / 60000f);
                }
        	}
        	else
        	{
                ...
                if(e.getName().equals("moveTo"))
                {
                	pathClosed = false;
                    e = e.element("pt");                        
                    path.moveTo(IntegerTool.parseIntWithoutException(e.attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(e.attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                }
                else if(e.getName().equals("lnTo"))
                {
                    e = e.element("pt");
                    path.lineTo(IntegerTool.parseIntWithoutException(e.attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(e.attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                }
                else if(e.getName().equals("quadBezTo"))
                {
                ...
                else if(e.getName().equals("cubicBezTo"))
                {
                    ...
                    path.cubicTo(IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        ...
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(2)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH,
                        IntegerTool.parseIntWithoutException(((Element)ptList.get(2)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                }
                else if(e.getName().equals("arcTo"))
                {
                    float wR = IntegerTool.parseIntWithoutException(e.attributeValue("wR")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
    ...
    public static ArrowPathAndTail getPathHeadArrowPath(Arrow arrow, int lineWidth, Element pathElement)
    {
        ...
        float p0X = IntegerTool.parseIntWithoutException(e.attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        float p0Y = IntegerTool.parseIntWithoutException(e.attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        float p1X = 0f, p1Y = 0f;
        ...
        {
            e = e.element("pt");
            p1X = IntegerTool.parseIntWithoutException(e.attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
            p1Y = IntegerTool.parseIntWithoutException(e.attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        ...
        else if(e.getName().equals("quadBezTo"))
        {
            ...
            if(ptList.size() == 2)
            {
                float ctrX1 = IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                float ctrY1 = IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                p1X = IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                p1Y = IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        ...
        else if(e.getName().equals("cubicBezTo"))
        {
            ...
            if(ptList.size() == 3)
            {
                float ctrX1 = IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                float ctrY1 = IntegerTool.parseIntWithoutException(((Element)ptList.get(0)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                float ctrX2 = IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                float ctrY2 = IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                p1X = IntegerTool.parseIntWithoutException(((Element)ptList.get(2)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                p1Y = IntegerTool.parseIntWithoutException(((Element)ptList.get(2)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
    ...
    public static ArrowPathAndTail getPathTailArrowPath(Arrow arrow, int lineWidth, Element pathElement)
    {
        ...
        {
            e = e.element("pt");
            p0X = IntegerTool.parseIntWithoutException(e.attributeValue("x")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
            p0Y = IntegerTool.parseIntWithoutException(e.attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        }
        else if(e.getName().equals("quadBezTo"))
        {
            List ptList = e.elements();
            if(ptList.size() == 2)
            {
                p0X = IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                p0Y = IntegerTool.parseIntWithoutException(((Element)ptList.get(1)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
            }
        ...
        else if(e.getName().equals("cubicBezTo"))
        {
            ...
            if(ptList.size() == 3)
            {
                p0X = IntegerTool.parseIntWithoutException(((Element)ptList.get(2)).attributeValue("x"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
                p0Y = IntegerTool.parseIntWithoutException(((Element)ptList.get(2)).attributeValue("y"))* MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
            }
        }
