...
//        processGroupShapeID(grpShape);
    ...
    public void setPageSize(Element slideSize)
    {
        ...
        if (slideSize != null)
        {
            pgx = FloatTool.parseFloatWithoutException(slideSize.attributeValue("cx"))
                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
            pgy = FloatTool.parseFloatWithoutException(slideSize.attributeValue("cy")) 
                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH;
        }
    ...
    private void processAnimation(PGSlide pgSlide, Element elem)
    {
        ...
        if(elem.element("txEl") != null && elem.element("txEl").element("pRg") != null)
        {
            ...
            pgSlide.addShapeAnimation(
                new ShapeAnimation(IntegerTool.parseIntWithoutException(shapeID), nType, IntegerTool.parseIntWithoutException(s), IntegerTool.parseIntWithoutException(e)));
        }
        else if(elem.element("bg") != null)
        {
            //background
            pgSlide.addShapeAnimation(
                new ShapeAnimation(IntegerTool.parseIntWithoutException(shapeID), nType, ShapeAnimation.Para_BG, ShapeAnimation.Para_BG));
        }
        else
        {
            pgSlide.addShapeAnimation(
                new ShapeAnimation(IntegerTool.parseIntWithoutException(shapeID), nType));
        }
    }
