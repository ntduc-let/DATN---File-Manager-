	...
	private static int matchOptionalFormatting(
		...
		if ((0 < formatting.length())
			...
			numberFormat.setMinimumIntegerDigits(
				IntegerTool.parseIntWithoutException(formatting.charAt(0) + ""));
			if ((2 < formatting.length())
				&& (formatting.charAt(1) == '.')
				&& Character.isDigit(formatting.charAt(2))) {
				numberFormat.setMaximumFractionDigits(
					IntegerTool.parseIntWithoutException(formatting.charAt(2) + ""));
			...
			if ((1 < formatting.length())
				...
				numberFormat.setMaximumFractionDigits(
					IntegerTool.parseIntWithoutException(formatting.charAt(1) + ""));
				numberFormat.format(number, outputTo, new FieldPosition(0));
				return 2;
			}
		}
