...
{    
    ...
    private short getColorIndex( Element clrElement)
    {
        ...
        if(clrElement != null)
        {
            if(clrElement.attribute("theme") != null)
            {
                ...
                int themeIndex = 0;
                try {
                    themeIndex = IntegerTool.parseIntWithoutException(clrElement.attributeValue("theme"));
                ...
                if(clrElement.attribute("tint") != null)
                {
                    ...
                    double tint = DoubleTool.parseDoubleWithoutException(clrElement.attributeValue("tint"));
                    //get color(argb)
            ...
            else if(clrElement.attribute("rgb") != null)
            {
                ...
                try {
                    index = IntegerTool.parseIntWithoutException(val, 16);
                } catch (Exception ignored) {
                }
            ...
            else if(clrElement.attribute("indexed") != null)
            {
                //get from paletter
                try {
                    index = IntegerTool.parseIntWithoutException(clrElement.attributeValue("indexed"));
                } catch (Exception ignored) {
                }
    ...
    private NumberFormat processNumberFormat(Element numFmt)
    {
        ...
        int Id = 0;
        try {
            Id = IntegerTool.parseIntWithoutException(numFmt.attribute("numFmtId").getValue());
        } catch (Exception ignored) {
        }
    ...
    private Font processFont(Element fontElement)
    {
        ...
        if (e != null)
        {
            sz = DoubleTool.parseDoubleWithoutException(e.attributeValue("val"));
        }
        font.setFontSize(sz);
    ...
    private BackgroundAndFill processFill(Element fillElement)
    {
        ...
        else if(fillElement.element("gradientFill") != null)
        {
        	...
        	for(int i = 0; i < gsLst.size(); i++)
        	{
        		...
        		positions[i] = FloatTool.parseFloatWithoutException(stop.attributeValue("position"));
            ...
            if(!"path".equalsIgnoreCase(gradientFill.attributeValue("type")))
        	{
                ...
                {
                    try {
                        degree = IntegerTool.parseIntWithoutException(gradientFill.attributeValue("degree"));
                    } catch (Exception ignored) {
    ...
    private void processCellStyleAlignment(CellStyle cellStyle, Element alignment)
    {
        ...
        {
            try {
                cellStyle.setRotation((short)IntegerTool.parseIntWithoutException(alignment.attributeValue("textRotation")));
            } catch (Exception ignored) {
            }
        }
        ...
        {
            try {
                cellStyle.setWrapText(IntegerTool.parseIntWithoutException(alignment.attributeValue("wrapText")) != 0);
            } catch (Exception ignored) {
            }
        }
        ...
        {
            try {
                cellStyle.setIndent((short)IntegerTool.parseIntWithoutException(alignment.attributeValue("indent")));
            } catch (Exception ignored) {
            }
        }
    }
    ...
    private CellStyle processCellStyle(Element styleElement)
    {
        ...
        try {
            idx = (value == null ? 0 : IntegerTool.parseIntWithoutException(value));
        } catch (Exception e) {
            idx = 0;
        }
