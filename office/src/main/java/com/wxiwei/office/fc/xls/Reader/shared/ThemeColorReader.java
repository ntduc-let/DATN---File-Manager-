...
public class ThemeColorReader
{
    ...
    private int getColorIndex(Element colorEle, Workbook book)
    {
        ...
        {
            try {
                color = IntegerTool.parseIntWithoutException(colorEle.element("srgbClr").attributeValue("val"), 16);
        ...
        {
            try {
                color = IntegerTool.parseIntWithoutException(colorEle.element("sysClr").attributeValue("lastClr"), 16);
            } catch (Exception ignored) {
            }
        }
