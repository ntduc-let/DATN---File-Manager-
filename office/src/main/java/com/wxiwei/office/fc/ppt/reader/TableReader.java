...
public class TableReader
{
        ...
        PGMaster master, Element tbl, Rectangle rect) throws Exception
    {
        ...
        if (tblGrid != null)
        {
            ...
            {
                int colWidth = (int)(IntegerTool.parseIntWithoutException(gridCol.attributeValue("w"))
            ...
            for (Element tr : trs)
            {
                int rowHeight = (int)(IntegerTool.parseIntWithoutException(tr.attributeValue("h"))
                    * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                if (rowHeight > 0)
                {
        ...
        for (Element tr : trs)
        {
            ...
            for (Element tc : tcs)
            {
                if (tc.attribute("hMerge") == null && tc.attribute("vMerge") == null)
                {
                    ...
                    {
                        int rowSpan = IntegerTool.parseIntWithoutException(tc.attributeValue("rowSpan"));
                    ...
                    {
                        int gridSpan = IntegerTool.parseIntWithoutException(tc.attributeValue("gridSpan"));
    ...
    public void processCellSection(IControl control, PGMaster master, TextBox tb, Rectangle rect, Element tc, TableCellStyle  cellStyle)
    {
        ...
        {    
            ...
            if (tcPr != null)
            {
        		...
        		{
        			leftMargin = (int)(IntegerTool.parseIntWithoutException(tcPr.attributeValue("marL")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH * MainConstant.PIXEL_TO_TWIPS);
        		}
        		//top margin
        		if(tcPr.attributeValue("marT") != null)
        		{
        			topMargin = (int)(IntegerTool.parseIntWithoutException(tcPr.attributeValue("marT")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH * MainConstant.PIXEL_TO_TWIPS);
        		}
        		//right margin
        		if(tcPr.attributeValue("marR") != null)
        		{
        			rightMargin = (int)(IntegerTool.parseIntWithoutException(tcPr.attributeValue("marR")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH * MainConstant.PIXEL_TO_TWIPS);
        		}
        		//bottom margin
        		if(tcPr.attributeValue("marB") != null)
        		{
        			bottomMargin = (int)(IntegerTool.parseIntWithoutException(tcPr.attributeValue("marB")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH * MainConstant.PIXEL_TO_TWIPS);
        		}
    ...
    public int processParagraph(IControl control, PGMaster master, SectionElement secElem, Element txBody, TableCellStyle  cellStyle)
    {
        ...
        if (bodyPr != null)
        {
            ...
            if (normAutofit != null && normAutofit.attribute("lnSpcReduction") != null)
            {
                ...
                {
                    lnSpcReduction = IntegerTool.parseIntWithoutException(val);
                }
            }
        }
