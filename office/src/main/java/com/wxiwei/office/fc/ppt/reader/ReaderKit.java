...
public class ReaderKit
{
        ...
        if (sp != null)
        {
            ...
            if (temp != null)
            {
                ...
                if (nvPr != null)
                {
                    ...
                    {
                        return (int) DoubleTool.parseDoubleWithoutException(ph.attributeValue("idx"));
                    }
                }
            }
        }
            ...
            if (off != null)
            {
                if (off.attribute("x") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            rect.x = (int)(IntegerTool.parseIntWithoutException(val) * zoomX
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                        else
                        {
                            rect.x = (int)(IntegerTool.parseIntWithoutException(val, 16) * zoomX
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                    }
                }
                if (off.attribute("y") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            rect.y = (int)(IntegerTool.parseIntWithoutException(val) * zoomY
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                        else
                        {
                            rect.y = (int)(IntegerTool.parseIntWithoutException(val, 16) * zoomY
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                    }
                }
            ...
            if (ext != null)
            {
                if (ext.attribute("cx") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            rect.width = (int)(IntegerTool.parseIntWithoutException(val) * zoomX
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                        else
                        {
                            rect.width = (int)(IntegerTool.parseIntWithoutException(val, 16) * zoomX
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                    }
                }
                if (ext.attributeValue("cy") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            rect.height = (int)(IntegerTool.parseIntWithoutException(val) * zoomY
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                        else
                        {
                            rect.height = (int)(IntegerTool.parseIntWithoutException(val, 16) * zoomY
                                * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
                        }
                    }
                }
    ...
    private int  processColorAttribute(Element colorE, int color, boolean isTableStyle)
    {
    	if (colorE.element("tint") != null)
        {
            if(isTableStyle)
            {
                color = ColorUtil.instance().getColorWithTint(color, 
                    1 - IntegerTool.parseIntWithoutException(colorE.element("tint").attributeValue("val")) / 100000.0);
            }
            else
            {
                color = ColorUtil.instance().getColorWithTint(color, 
                    IntegerTool.parseIntWithoutException(colorE.element("tint").attributeValue("val")) / 100000.0);
            }
        ...
        {
            color = ColorUtil.instance().getColorWithTint(color, 
                IntegerTool.parseIntWithoutException(colorE.element("lumOff").attributeValue("val")) / 100000.0);
        }
        else if (colorE.element("lumMod") != null)
        {
            color = ColorUtil.instance().getColorWithTint(color, 
                IntegerTool.parseIntWithoutException(colorE.element("lumMod").attributeValue("val")) / 100000.0 - 1);
        }
        else if (colorE.element("shade") != null)
        {
            color = ColorUtil.instance().getColorWithTint(color, 
                -IntegerTool.parseIntWithoutException(colorE.element("shade").attributeValue("val")) / 200000.0);
        }
        ...
        if(colorE.element("alpha") != null)
        {
            ...
            if(val != null)
            {
                int alpha = (int)(IntegerTool.parseIntWithoutException(val) / 100000f * 255);
    ...
    public int getColor(PGMaster master, Element solidFill, boolean isTableStyle)
    {
        if (solidFill != null)
        {
            ...
            {
                int r = IntegerTool.parseIntWithoutException(temp.attributeValue("r")) * 255 / 100;
                int g = IntegerTool.parseIntWithoutException(temp.attributeValue("g")) * 255 / 100;
                int b = IntegerTool.parseIntWithoutException(temp.attributeValue("b")) * 255 / 100;
        ...
        if (temp != null)
        {
            ...
            if (nvPr != null)
            {
                ...
                else if (nvPr.attribute("userDrawn") != null)
                {
                    ...
                    if ((val != null && val.length() > 0 && IntegerTool.parseIntWithoutException(val) > 0))
                    {
                        return true;
                    }
                }
            ...
            if (ext != null)
            {
                if (ext.attribute("cx") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            grpWidth = IntegerTool.parseIntWithoutException(val);
                        }
                        else
                        {
                            grpWidth = IntegerTool.parseIntWithoutException(val, 16);
                        }
                    }
                }
                if (ext.attributeValue("cy") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            grpHeight = IntegerTool.parseIntWithoutException(val);
                        }
                        else
                        {
                            grpHeight = IntegerTool.parseIntWithoutException(val, 16);
                        }
                    }
                }
            }
            ...
            if (ext != null)
            {
                if (ext.attribute("cx") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            childWidth = IntegerTool.parseIntWithoutException(val);
                        }
                        else
                        {
                            childWidth = IntegerTool.parseIntWithoutException(val, 16);
                        }
                    }
                }
                if (ext.attributeValue("cy") != null)
                {
                    ...
                    if (val != null && val.length() > 0)
                    {
                        if(isDecimal(val))
                        {
                            childHeight = IntegerTool.parseIntWithoutException(val);
                        }
                        else
                        {
                            childHeight = IntegerTool.parseIntWithoutException(val, 16);
                        }
                    }
                }
            }
    ...
    public void processRotation(IShape shape, Element xfrm)
    {
        if (xfrm != null)
        {
            ...
            if (xfrm.attribute("flipH") != null)
            {
                ...
                if (val != null && val.length() > 0 && IntegerTool.parseIntWithoutException(val) == 1)
                {
            ...
            if (xfrm.attribute("rot") != null)
            {
                ...
                if (val != null && val.length() > 0)
                {
                    shape.setRotation(FloatTool.parseFloatWithoutException(val) / 60000);
                }
            }
        }
    }
    ...
    public boolean isHidden(Element sp)
    {
        ...
        if (temp != null)
        {
            ...
            if (cNvPr != null && cNvPr.attribute("hidden") != null
                && IntegerTool.parseIntWithoutException(cNvPr.attributeValue("hidden")) > 0)
            {
                return true;
            }
        }
