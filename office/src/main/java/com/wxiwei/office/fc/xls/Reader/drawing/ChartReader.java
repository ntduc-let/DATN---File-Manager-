...
public class ChartReader
{
    ...
    private float getTextSize(Element txPr)
    {
        		...
        		&& (ele = ele.element("defRPr")) != null)
        {
        	...
        	{
        		fontSize = IntegerTool.parseIntWithoutException(sz) / 100f;
        	}
        }
    ...
    private int getColor(Element solidFillElement)
    {
        ...
        if(solidFillElement.element("srgbClr") != null)
        {
            ...
            color = IntegerTool.parseIntWithoutException(val, 16);
            color = (0xFF << 24) | color;
        }
        else if(solidFillElement.element("schemeClr") != null)
        {
            ...
            {
                double tint = IntegerTool.parseIntWithoutException(clr.element("lumMod").attributeValue("val")) / 100000.0;
                if(clr.element("lumOff") != null)
                {
                    tint = IntegerTool.parseIntWithoutException(clr.element("lumOff").attributeValue("val")) / 100000.0;
                }
                else
                {
                    tint = tint - 1; 
...
//            type = AbstractChart.CHART_BUBBLE;
    ...
    private void getMaxMinValue(Element scaling, short axPos)
    {
    	switch(axPos)
		{
			...
			case AxisPosition_Right:
		        ...
		        {
		            hasMaxY = true;
		            maxY = DoubleTool.parseDoubleWithoutException(scaling.element("max").attributeValue("val"));
		        }
		        
		        if(scaling.element("min") != null)
		        {
		            hasMinY = true;
		            minY = DoubleTool.parseDoubleWithoutException(scaling.element("min").attributeValue("val"));
		        }   
				break;
			...
			case AxisPosition_Bottom:
		        ...
		        {
		            hasMaxX = true;
		            maxX = DoubleTool.parseDoubleWithoutException(scaling.element("max").attributeValue("val"));
		        }
		        
		        if(scaling.element("min") != null)
		        {
		            hasMinX = true;
		            minX = DoubleTool.parseDoubleWithoutException(scaling.element("min").attributeValue("val"));
		        }   
				break;
		}
    }
    ...
    private void setSeriesRendererProp(XYMultipleSeriesRenderer renderer, Element series, PointStyle[] styles)
    {
        ...
        int seriesOrder = IntegerTool.parseIntWithoutException(series.element("order").attributeValue("val"));
    ...
    private Object getSeries(Element series, short chartType)
    {
        ...
        {
        	seriesName = "Series " + (IntegerTool.parseIntWithoutException(series.element("order").attributeValue("val")) + 1);
        }         
        ...
        if(chartType != AbstractChart.CHART_SCATTER)
        {
        	if(series.element("val") != null)
            {
                ...
                {
                    numPoint = iter.next();
                    value = DoubleTool.parseDoubleWithoutException(numPoint.element("v").getText());
                    aSeries.add(value);
                }
        ...
        {        	
        	...
        	if(xNumber != null && yNumber != null)
        	{
                ...
                while(iterX.hasNext() && iterY.hasNext())
                {
                	...
                	valueX = DoubleTool.parseDoubleWithoutException(xNumPoint.element("v").getText());
                	valueY = DoubleTool.parseDoubleWithoutException(yNumPoint.element("v").getText());
    ...
    protected CategorySeries buildCategoryDataset(Element chart, DefaultRenderer renderer) 
    {
        ...
        {  
            ...
            if(series.element("val") != null)
            {
                ...
                {
                    pt = iter.next();
                    valList.add(DoubleTool.parseDoubleWithoutException(pt.element("v").getText()));
                }  
            }  
