...
public class ShaderKit
{
	...
	public static Gradient readGradient(PGMaster master, Element gradFill)
    {
    	...
    	for(int i = 0; i < gsCnt; i++)
    	{
    		...
    		positions[i] = IntegerTool.parseIntWithoutException(pos) / 100000.f;
    	...
    	if(ele != null)
    	{
    		...
    		float angle = (float) IntegerTool.parseIntWithoutException(ele.attributeValue("ang")) / 60000;
	...
	public static Gradient readGradient(Map<String, Integer> schemeColor, Element gradFill)
    {
    	...
    	{
    		//linear gradient property
    		float angle = IntegerTool.parseIntWithoutException(ele.attributeValue("ang")) / 60000;
        	return new LinearGradientShader(angle, colors, positions);
    	}
    	else if((ele = gradFill.element("path")) != null)
    	{
    ...
    public static TileShader readTile(Picture picture, Element tile)
    {
    	...
    	float horiRatio = IntegerTool.parseIntWithoutException(tile.attributeValue("sx")) / 100000.f;
    	float vertRatio = IntegerTool.parseIntWithoutException(tile.attributeValue("sy")) / 100000.f;
    	int offsetX = Math.round(IntegerTool.parseIntWithoutException(tile.attributeValue("tx")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
    	int offsetY = Math.round(IntegerTool.parseIntWithoutException(tile.attributeValue("ty")) * MainConstant.PIXEL_DPI / MainConstant.EMU_PER_INCH);
