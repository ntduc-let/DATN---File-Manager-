...
import com.ntduc.baseproject.databinding.FragmentListAudioSafeBinding
...
class ListAudioSafeFragment : BaseFragment<FragmentListAudioSafeBinding>(R.layout.fragment_list_audio_safe) {
    ...
    private fun showMenu(view: View, file: File) {
        ...
        popupBinding.delete.setOnClickListener {
            ...
            val audioFolder = File(Environment.getExternalStorageDirectory().path + "/.${getString(R.string.app_name)}/.SafeFolder/audio")
            File("${audioFolder.path}/${file.name}").delete(requireContext())
    ...
    override fun addObservers() {
        super.addObservers()
        observe(viewModel.audioSafeListLiveData, ::handleAudioList)
    }

    private fun handleAudioList(status: Resource<List<File>>) {
        when (status) {
            is Resource.Loading -> {
                binding.rcv.gone()
