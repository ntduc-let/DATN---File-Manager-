...
import com.ntduc.baseproject.databinding.FragmentListApkSafeBinding
...
class ListApkSafeFragment : BaseFragment<FragmentListApkSafeBinding>(R.layout.fragment_list_apk_safe) {
    ...
    private fun showMenu(view: View, file: File) {
        ...
        popupBinding.delete.setOnClickListener {
            ...
            val apkFolder = File(Environment.getExternalStorageDirectory().path + "/.${getString(R.string.app_name)}/.SafeFolder/apk")
            File("${apkFolder.path}/${file.name}").delete(requireContext())
    ...
    override fun addObservers() {
        super.addObservers()
        observe(viewModel.apkSafeListLiveData, ::handleApkList)
    }

    private fun handleApkList(status: Resource<List<File>>) {
        when (status) {
            is Resource.Loading -> {
                binding.rcv.gone()
